                  ...
                  inout standard_metadata_t standard_metadata)
{
    ...
    action phiPosPos(int<32> sin, int<32> cos) {
        meta.sinPhi = sin;
        meta.cosPhi = cos;
    }

    action phiNegPos(int<32> sin, int<32> cos) {
        meta.sinPhi = -sin;
        meta.cosPhi = cos;
    }

    action phiPosNeg(int<32> sin, int<32> cos) {
        meta.sinPhi = sin;
        meta.cosPhi = -cos;
    }

    action phiNegNeg(int<32> sin, int<32> cos) {
        meta.sinPhi = -sin;
        meta.cosPhi = -cos;
    }

    action thetaPosPos(int<32> sin, int<32> cos) {
        meta.sinTheta = sin;
        meta.cosTheta = cos;
    }

    action thetaNegPos(int<32> sin, int<32> cos) {
        meta.sinTheta = -sin;
        meta.cosTheta = cos;
    }

    action thetaPosNeg(int<32> sin, int<32> cos) {
        meta.sinTheta = sin;
        meta.cosTheta = -cos;
    }

    action thetaNegNeg(int<32> sin, int<32> cos) {
        meta.sinTheta = -sin;
        meta.cosTheta = -cos;
    }
    ...
    action noMatch(bit<32> x) {
    }


    table phiTable {
    	key = {
    		hdr.spherical.phi : exact;
    	}
    	actions = {
    		phiPosPos;
    		phiNegPos;
        ...
        size = (1 << (FIX_POINT_EXPONENT + 3));
    }

    table thetaTable {
    	key = {
    		hdr.spherical.theta : exact;
    	}
    	actions = {
    		thetaPosPos;
    		thetaNegPos;
	...
	apply {
        if (hdr.arp.isValid()) {
            ...
            int<32> reg;
            multiply01(reg, meta.sinTheta, hdr.spherical.radius);

            int<32> x = 0;
            int<32> y = 0;
            int<32> z = 0;

            multiply02(x, reg, meta.cosPhi);
            multiply03(y, reg, meta.sinPhi);
            multiply04(z, hdr.spherical.radius, meta.cosTheta);
